...
export const NeutralCalculate = ({
  ...
  const handleClickForbiddenCard = (forbiddenCount: number) => {
    ...
    setForbidden((forbidden) => [...forbidden, generateRandom0to2()]);
    setForbiddenCount((forbiddenCount) => forbiddenCount + 1);
    setData(data + 30);
  };
  ...
  function deleteForbiddenCard(index: number) {
    const newArrayForbiddenCard = forbidden.filter(
      (i: number, idx: number) => idx !== index
    );
    setForbidden(newArrayForbiddenCard);
  }

  function deleteMonsterCard(index: number) {
    const newArrayMonsterCard = monsterArray.filter(
      (i: number, idx: number) => idx !== index
    );
  ...
  function deleteNeutralCard(index: number) {
    const newArrayNeutralCard = ArrayNeutralCard.filter(
      (i: number, idx: number) => idx !== index
    );
  ...
  function toggleEpiphanyActive(index: number) {
    setNeutralActiveEpiphany((neutralActiveEpiphany) =>
      neutralActiveEpiphany.includes(index)
        ? neutralActiveEpiphany.filter((i) => i !== index)
        : [...neutralActiveEpiphany, index]
    );
  }
  ...
  function toggleDivineActive(index: number) {
    setNeutralActiveDivine((neutralActiveDivine) =>
      neutralActiveDivine.includes(index)
        ? neutralActiveDivine.filter((i) => i !== index)
        : [...neutralActiveDivine, index]
    );
  }
  ...
  function isEpiphany(index: number) {
    if (neutralEpiphanyArray.includes(index)) {
      setData((data: number) => data - 10);
      const newActiveEpiphany = neutralActiveEpiphany.filter(
        (i) => i !== index
      );
  ...
  function handleClickAddCardNeutral(card: number, index: number) {
    setArrayNeutralCard([
      ...ArrayNeutralCard.slice(0, index),
      card,
      ...ArrayNeutralCard.slice(index),
    ]);
    if (neutralDivineArray.includes(index)) {
      setData((data: number) => data + 20);
      setNeutralActiveDivine((neutralActiveDivine) => [
      ...
      setNeutralDivineArray((neutralDivineArray) => [
        ...neutralDivineArray,
        index + 1,
      ]);
      ...
      setNeutralEpiphanyArray((neutralEpiphanyArray) => [
        ...neutralEpiphanyArray,
        index + 1,
      ]);
    }
  ...
  return (
    <Fragment>
      {/*Neutral cards abajo   */}
      {ArrayNeutralCard.map((cardNeutral: number, i: number) => (
      ...
      {monsterArray.map((cardMonster: number, i: number) => (
        <div key={i} className="relative">
          <img
      ...
      {forbidden.map((cardForbidden: number, i: number) => (
        <div key={i} className="relative">
          <img
